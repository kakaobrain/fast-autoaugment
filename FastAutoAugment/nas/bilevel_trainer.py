...
class BilevelTrainer(Trainer):
    def __init__(self, conf_train:Config, model:Model, device)->None:
        ...
        conf_bilevel = conf_train['bilevel']
        self.max_final_edges = conf_bilevel['max_final_edges']
        self.plotsdir = conf_bilevel['plotsdir']
        self.conf_w_optim = conf_train['optimizer']
        w_momentum = self.conf_w_optim['momentum']
        w_decay = self.conf_w_optim['decay']
        conf_lossfn   = conf_train['lossfn']
        conf_alpha_optim = conf_bilevel['alpha_optimizer']

        lossfn = utils.get_lossfn(conf_lossfn).to(device)
        alpha_optim = utils.get_optimizer(conf_alpha_optim, model.alphas())
        self._bilevel_optim = _BilevelOptimizer(w_momentum, w_decay, alpha_optim,
                                                model, lossfn)

    @overrides
    def get_optimizer(self)->Optimizer:
        return utils.get_optimizer(self.conf_w_optim, self.model.weights())
