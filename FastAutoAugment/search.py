...
from FastAutoAugment.common import get_logger, add_filehandler, common_init
...
def _model_save_path(logdir, dataset, model, tag):
    return os.path.join(logdir, '%s_%s_%s.model' \
        % (dataset, model, tag))     # TODO
...
if __name__ == '__main__':
    ...
    save_paths = [_model_save_path(args.logdir, C.get()['dataset'], C.get()['model']['type'], 'ratio%.1f_fold%d' \
                % (args.cv_ratio, i)) for i in range(cv_num)]
    print(save_paths)
    ...
    reqs = [
        train_model.remote(copy.deepcopy(copied_c), args.dataroot, C.get()['aug'], args.cv_ratio, i, 
            save_path=save_paths[i], skip_exist=True)
        ...
        for cv_fold in range(cv_num):
            ...
            exp_config = {
                name: {
                    ...
                    'config': {
                        'dataroot': args.dataroot, 'save_path': save_paths[cv_fold],
    ...
    num_experiments = 5
    default_path = [_model_save_path(args.logdir, C.get()['dataset'], C.get()['model']['type'], 'ratio%.1f_default%d' % (args.cv_ratio, _)) for _ in range(num_experiments)]
    augment_path = [_model_save_path(args.logdir, C.get()['dataset'], C.get()['model']['type'], 'ratio%.1f_augment%d' % (args.cv_ratio, _)) for _ in range(num_experiments)]
